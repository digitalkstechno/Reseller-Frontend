        ...
        {lead && (
          <div className="space-y-4 text-sm pr-1">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              />
              {/* Amount Summary Card - Total / Paid / Pending in one block matching screenshot design */}
              <div className="md:col-span-2 rounded-xl bg-gray-50/90 border border-gray-200/80 px-4 py-3.5">
                {(() => {
                  const totalAmt = Number((lead as any).paymentAmount) || Number((lead as any).projectAmount) || 0;
                  const paidAmt = (lead as any).payments && Array.isArray((lead as any).payments) && (lead as any).payments.length > 0
                  ...
                  return (
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-bold text-gray-900">{formatIndianCurrency(totalAmt)}</span>
                      </div>

                      {/* Paid Amount — middle with rupee icon */}
                      <div className="flex items-center justify-between">
                        <span className="flex items-center gap-2 text-xs font-medium text-gray-600">
                          <span className="text-gray-400 font-bold text-xs w-4 text-center flex-shrink-0">₹</span>
                          Paid Amount
                        </span>
                        <span className="text-sm font-semibold text-emerald-600">{formatIndianCurrency(paidAmt)}</span>
                      </div>
                      ...
                      {/* Pending Amount — last */}
                      <div className="flex items-center justify-between">
                        <span className={`flex items-center gap-2 text-xs font-bold ${pendingAmt > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
                          <span className="font-bold text-xs w-4 text-center flex-shrink-0">₹</span>
                          {pendingAmt > 0 ? 'Pending Amount' : 'Total Paid'}
                        </span>
                        <span className={`text-sm font-bold ${pendingAmt > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
                          {formatIndianCurrency(pendingAmt)}
                        </span>
                      </div>
                    </div>
                  );
